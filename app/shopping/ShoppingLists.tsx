...
export function ShoppingLists() {
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="glass rounded-2xl p-4 space-y-3 animate-slide-down">
          <input
            ...
            className="glass-input w-full px-4 py-2.5 rounded-xl text-base outline-none transition"
            autoFocus
          />
          <input
            type="date"
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowForm(false)} className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white/50 bg-white/[0.06] hover:bg-white/[0.1] transition">
              Annuler
            </button>
            <button
              onClick={handleCreate}
              disabled={submitting || !newName.trim()}
              className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-500 transition disabled:opacity-50"
            >
              {submitting ? '…' : 'Créer'}
            </button>
          </div>
      ...
      {lists.length === 0 ? (
        <p className="text-white/30 text-sm text-center py-10">Aucune liste pour l&apos;instant.</p>
      ) : (
        <ul className="space-y-3 stagger">
          {lists.map((list) => (
            <li
              ...
              className="glass rounded-2xl p-4 flex items-center justify-between cursor-pointer active:scale-[0.97] transition-all duration-150 select-none hover:border-white/[0.14]"
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1.5 rounded-xl text-white/20 hover:text-red-400 hover:bg-red-500/10 transition"
                >
